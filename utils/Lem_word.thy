...
fun cleanBitSeq  :: " bitSequence \<Rightarrow> bitSequence "  where 
    ...
    None => (BitSeq len s (List.rev (dropWhile ((iff) s) (List.rev bl))))
  | Some n  => (BitSeq len s (List.rev (dropWhile ((iff) s) (List.rev (List.take (n-( 1 :: nat)) bl)))))
))" 
...
definition bitSeqMult  :: " bitSequence \<Rightarrow> bitSequence \<Rightarrow> bitSequence "  where 
     " bitSeqMult bs1 bs2 = ( bitSeqArithBinOp times bs1 bs2 )"
